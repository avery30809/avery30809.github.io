document.addEventListener("DOMContentLoaded", () => {
    ...
    function getClassroomName() {
        ...
        .then(datas => {
            datas.forEach((data) => {
                ...
                inquiryRoom.value = allClassroomName[0];
                let room = [document.createElement("li"), document.createElement("li"), document.createElement("li")];
                room[0].innerHTML = data[0];
                room[1].innerHTML = data[0];
                room[2].innerHTML = data[0];
                ...
                room[2].addEventListener("click", ()=>{
                    inquiryRoom.value = room[2].innerHTML;
                    getAllowedHistory();
                }, false);
                document.getElementById("classRoomSelect").appendChild(room[0]);
                document.getElementById("fastRoomSelect").appendChild(room[1]);
                document.getElementById("inquiryRoomSelect").appendChild(room[2]);
            });
        });
    }
    ...
    document.getElementById("logoutBtn").addEventListener("click", ()=>{
        fetch("ntoucsroomlease.infinityfreeapp.com/Controller/Api/UserController.php?action=logout");
    ...
    document.addEventListener("click", (event) => {
        ...
        document.querySelectorAll(".inquiryDropdown").forEach((element) => {
            element.classList.remove("show");
        });
    }, false);
    ...
    fastForm.addEventListener("submit", (event) => {
        ...
        fetch(`ntoucsroomlease.infinityfreeapp.com/Controller/Api/UserController.php?account=${myForm.get("userAccount")}&action=getID`)
    ...
    */
    ...
    function getHistoryForm() {
        fetch("ntoucsroomlease.infinityfreeapp.com/Controller/Api/HistoryController.php?action=getHistoryForm")
        .then(response => response.json())
        .then(datas => {
            //console.log(datas);
    ...
    function getKeyRecord() {
        ...
        fetch("ntoucsroomlease.infinityfreeapp.com/Controller/Api/HistoryController.php?action=getKeyRecord")
        .then(response=>response.json())
        .then(datas=>{
            if(datas.error === undefined) {
                ...
                datas.forEach(data=>{
                    content += `<div class="keyRecordContent">
                                    ...
                                    <span style='width:15%;'>${data[1]} ${data[9]} ${data[10] ?? ""}</span>
    ...
    const inquiryRoom = document.getElementById("inquiryRoomBox"),
            inquiryDropdownRoom = document.getElementById("inquiryDropdownRoom");
    let inquiryResult = document.querySelector(".classroomInquiryResult")

    document.getElementById("inquiryRoomBox").addEventListener("click", (event) => {
        event.stopPropagation();
        event.target.parentElement.classList.toggle("checked");
        inquiryDropdownRoom.classList.toggle("show");
        document.querySelectorAll(".checked").forEach((element) => {
            if (element != event.target.parentElement)
                element.classList.remove("checked");
        });
    ...
    function getAllowedHistory() {
        let content = "";
        fetch("ntoucsroomlease.infinityfreeapp.com/Controller/Api/HistoryController.php?action=getAllowedHistory")
        .then(response=>response.json())
        .then(datas=>{
            if (datas.error !== undefined) {
                    ...
                    content += `<div class="content">
                                    ...
                                    <div class="text" data-index="inquiry ${index}">
                                        ...
                                        <div class="accounting">
                                            ...
                                            <p>第${data[3]}堂</p>
                                            <p>${data[4] === 9 ? "第9堂後" : `第${data[4]}堂`}</p>
                                            <button class="handleCancelButton" data-argument = "${data[1]} ${data[0]} ${data[2]} ${data[3]} ${data[4]}">取消</button>
                                        </div>
                                    </div>
                                </div>`;
                }
            })
            inquiryResult.innerHTML = content;
            document.querySelectorAll('.inquiryImg').forEach(img => {
                img.addEventListener('click', () => {
                    const index = img.getAttribute('data-index');
                    let formContent = document.querySelector(`.text[data-index="inquiry ${index}"]`);
                    formContent.classList.toggle("show");
                    img.classList.toggle("show");
                });
            });
            document.querySelectorAll('.handleCancelButton').forEach(btn => {
                btn.addEventListener('click', (event) => {
                    let arg = event.target.getAttribute("data-argument").split(" ");
                    let testForm = new FormData();
                    testForm.append("roomName", arg[0]);
